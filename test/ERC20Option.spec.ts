...
describe("unit:ERC20Option", function () {
    ...
    let ONE = ethers.utils.parseUnits("1", 18);
    let TWO = ethers.utils.parseUnits("2", 18);
    let timeInterval = BigNumber.from("100");
  describe("#mintOption", function () {
    ...
    it("should mint tokens and send to user", async function () {
      const depositor = Signers[1];
      await expect(ERC20Option.connect(depositor).mintOption(1, ONE, depositor.address)).to.emit(ERC20Option, "NewDeposit");
